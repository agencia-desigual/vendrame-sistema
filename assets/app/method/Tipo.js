 ...
 */
$("#formInserirTipo").on("submit", function(){
    ...
    // Recupera o url
    var url = Global.config.urlApi + "tipo/insert";
    ...
    Global.enviaApi("POST", url, form,token.token)
        .then((data) => {
            ...
            // Limpa o formulário
            Global.limparFormulario("#formInserirTipo");

            // Envia para o editar
            setTimeout(() => {
                location.href = Global.config.url + "painel/tipo/adicionar";
            },800);
        })
        .catch((error) => {
...
$(".deletarTipo").on("click", function () {
    ...
    // Url e Token
    var url = Global.config.urlApi + "tipo/delete/" + id;
    var token = Global.session.get("token");

    // Pergunta se realmente quer deletar
    Swal.fire({
        title: "Deletar Tipo",
        text: "Realmente deseja deletar esse tipo?",
        type: 'warning',
 ...
 * enviado seus dados para a API correspndente.
 */
$("#formAlterarTipo").on("submit", function(){
    ...
    // Recupera o url
    var url = Global.config.urlApi + "tipo/update/"+id;
...
$(".selecionaMarca").on("change", function () {
    ...
    // Url
    var url = Global.config.url + "painel/tipo/adicionar?marca=" + marca;
