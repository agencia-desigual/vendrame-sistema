 ...
 */
$("#formInserirAtributo").on("submit", function(){
    ...
    // Recupera o url
    var url = Global.config.urlApi + "atributo/insert";
    ...
    Global.enviaApi("POST", url, form,token.token)
        .then((data) => {
            ...
            // Limpa o formulário
            Global.limparFormulario("#formInserirAtributo");
 ...
 */
$(".deletarAtributo").on("click", function () {
    ...
    // Url e Token
    var url = Global.config.urlApi + "atributo/delete/" + id;
    var token = Global.session.get("token");

    // Pergunta se realmente quer deletar
    Swal.fire({
        title: "Deletar Atributo",
        text: "Realmente deseja deletar esse atributo?",
        type: 'warning',
 ...
 * enviado seus dados para a API correspndente.
 */
$("#formAlterarAtributo").on("submit", function(){
    ...
    // Recupera o url
    var url = Global.config.urlApi + "atributo/update/"+id;
