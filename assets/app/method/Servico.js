 ...
 */
$("#formInserirServico").on("submit", function(){
    ...
    // Recupera o url
    var url = Global.config.urlApi + "servico/insert";
    ...
    Global.enviaApi("POST", url, form,token.token)
        .then((data) => {
            ...
            // Atualiza a página
            setTimeout(() => {
                location.href = Global.config.url + "painel/servico/alterar/" + data.objeto.id_servico;
            }, 1000);
 ...
 */
$(".deletarServico").on("click", function () {
    ...
    // Url e Token
    var url = Global.config.urlApi + "servico/delete/" + id;
    var token = Global.session.get("token");

    // Pergunta se realmente quer deletar
    Swal.fire({
        title: "Deletar Serviço",
        text: "Realmente deseja deletar esse serviço?",
        type: 'warning',
 ...
 * enviado seus dados para a API correspndente.
 */
$("#formAlterarServico").on("submit", function(){
    ...
    // Recupera o url
    var url = Global.config.urlApi + "servico/update/"+id;
